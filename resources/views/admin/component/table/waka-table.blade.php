<div class="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
    ...
    <div class="p-5">
         @include('admin.component.button.waka-button')
    </div>
            ...
            <tbody>
                @forelse($wakas as $index => $waka)
                    <tr class="border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-white/[0.02]">
                        ...
                        <td class="px-5 py-4 sm:px-6">
                            <p class="font-medium text-gray-800 text-theme-sm dark:text-white/90">{{ $waka->name }}</p>
                        </td>
                        <td class="px-5 py-4 sm:px-6">
                            <p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ $waka->email }}</p>
                        </td>
                        <td class="px-5 py-4 sm:px-6">
                            <p class="text-gray-500 text-theme-sm dark:text-gray-400">
                                {{ $waka->number_phone ?? '-' }}
                        ...
                        <td class="px-5 py-4 sm:px-6">
                            <div class="flex items-center gap-2">
                                {{-- Tombol Edit --}}
                                <a href="{{ route('admin.waka.edit', $waka->id) }}"
                                ...
                                {{-- Tombol Hapus --}}
                                <form action="{{ route('admin.waka.destroy', $waka->id) }}" method="POST"
                                    onsubmit="return confirm('Yakin ingin menghapus waka ini?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-5 py-8 text-center text-gray-400 text-theme-sm dark:text-gray-500">
                            Belum ada data waka.
                        </td>
                    </tr>
                @endforelse
